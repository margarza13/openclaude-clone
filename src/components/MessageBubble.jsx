...
// Inline syntax highlighter using highlight.js loaded via CDN
// We dynamically load hljs to avoid adding a heavy npm dep
function CodeBlock({ lang, code }) {
  ...
  useEffect(() => {
    ...
    if (window.hljs) {
      ...
      // Load highlight.js CSS + JS dynamically once
  ...
  return (
    <div className="my-3 rounded-xl overflow-hidden border border-[#0f3460]">
      <div className="flex items-center justify-between px-4 py-1.5 bg-[#0f3460]/70 border-b border-[#0f3460]">
        <span className="text-xs text-gray-400 font-mono">{lang || 'code'}</span>
        <CopyButton text={code} />
      </div>
      <pre className="m-0 rounded-b-xl overflow-x-auto text-sm">
...
function inlineMarkdown(text) {
  ...
  return parts.map((part, idx) => {
    if (part.startsWith('`') && part.endsWith('`') && part.length > 2) {
      return (
        <code key={idx} className="bg-[#0a0a1a] text-green-300 px-1.5 py-0.5 rounded text-xs font-mono">
          {part.slice(1, -1)}
        </code>
      );
    }
...
function renderMarkdown(text) {
  ...
  while (i < lines.length) {
    ...
    // Headings
    if (line.startsWith('### ')) {
      elements.push(<h3 key={i} className="text-base font-semibold text-white mt-3 mb-1">{inlineMarkdown(line.slice(4))}</h3>);
      i++; continue;
    }
    if (line.startsWith('## ')) {
      elements.push(<h2 key={i} className="text-lg font-bold text-white mt-4 mb-1">{inlineMarkdown(line.slice(3))}</h2>);
      i++; continue;
    }
    if (line.startsWith('# ')) {
    ...
    if (line.match(/^[-*] /)) {
      ...
      elements.push(<ul key={`ul-${i}`} className="my-2 space-y-1 text-sm pl-2">{listItems}</ul>);
      continue;
    }
    ...
    if (line.match(/^\d+\. /)) {
      ...
      elements.push(<ol key={`ol-${i}`} className="my-2 space-y-1 text-sm pl-2">{listItems}</ol>);
      continue;
    }
